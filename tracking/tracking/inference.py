...
def constructBayesNet(gameState: hunters.GameState):
    ...
    temp1 = [PAC,GHOST0,GHOST1,OBS0,OBS1]
    variables.extend(temp1)
    #2. Pacman and the two ghosts can be anywhere in the grid (we ignore walls for this).
    #Add all possible position tuples for these.
    edge_1 = (GHOST0,OBS0)
    edge_2 = (PAC,OBS0)
    edge_3 = (PAC,OBS1)
    edge_4 = (GHOST1,OBS1)
    temp2 = [edge_1,edge_2,edge_3,edge_4]
    edges.extend(temp2)

    #3.Observations here are non-negative, equal to Manhattan distances of Pacman to ghosts
    # noise.

    # Agent positions
    agent_positions = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    max_possible_distance = X_RANGE + Y_RANGE - 2  # Max possible Manhattan distance in the grid
    observation_domain = list(range(max_possible_distance + MAX_NOISE + 1))
    # Assign domains
    for var in [PAC, GHOST0, GHOST1]:
        variableDomainsDict[var] = agent_positions

    for var in [OBS0, OBS1]:
        variableDomainsDict[var] = observation_domain
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # params: bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]
        # initialize return variables and the variables to eliminate
        # evidenceVariablesSet = set(evidenceDict.keys())
        # queryVariablesSet = set(queryVariables)
        # eliminationVariables = (bayesNet.variablesSet() - evidenceVariablesSet) - queryVariablesSet
        ...
        #     newFactors = []
        #     factorwithvar = [factor for factor in currentFactorsList if var in factor.variablesSet()]
        #     if factorwithvar:
        #         joinedFactor = joinFactors(factorwithvar)
        #         newFactors.append(joinedFactor)
        #
        #     # Eliminate the variable from the joined factor
        #     for factor in newFactors:
        #         if len(factor.unconditionedVariables()) > 1 and var not in factor.unconditionedVariables():
        #             eliminatedFactor = eliminate(factor,var)
        #             currentFactorsList.append(eliminatedFactor)
        #     # Remove the old factors and replace them with the new, processed ones
        #     currentFactors = [factor for factor in currentFactorsList if var not in factor.variablesSet()] + newFactors
        #     # print("currentFactors: ", currentFactors)
        #
        # # after processing all hidden variables, get all the remaining vars joined
        # finalFactor = joinFactors(currentFactors)
        # # normalize so that the probability sums to one
        # # the input factor contains only the query variables and the evidence variables,
        # # both as unconditioned variables
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        # noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        # raiseNotDefined()
        # input: observation: int, gameState: busters.GameState
        # create the beliefs
        # store = DiscreteDistribution()
        # get positions
        pacman_loc = gameState.getPacmanPosition()
        jail_loc = self.getJailPosition()
        # ghost positions:
        for pos in self.allPositions:
            prob = self.getObservationProb(observation, pacman_loc,pos,jail_loc)
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        extra_particles = nums % len(legal_positions)
        for position in legal_positions:
            # times for each position
            for _ in range(nums//len(legal_positions)):
                self.particles.append(position)
        #for the left particles
        for i in range(extra_particles):
            self.particles.append(legal_positions[i])
        # raiseNotDefined()
        "*** END YOUR CODE HERE ***"
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        normalized_distribution = DiscreteDistribution()
        for pos in self.particles:
            normalized_distribution[pos] += 1
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # to construct a weight distribution over self.particles
        ...
        for ghost_pos in self.particles:
            ...
            distribution[ghost_pos]+=prob
        # special case: all particles receive zero weight
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # construct a new list of particles (next state of current particles)
        new_particles = {}

        for i in range(self.numParticles):
            oldPos = self.particles[i]
            if oldPos in new_particles:
                self.particles[i] = new_particles[oldPos].sample()
            else:
                newPosDist = self.getPositionDistribution(gameState, oldPos)
                new_particles[oldPos] = newPosDist
